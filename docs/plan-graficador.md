...
| 3 | Integración: el panel entra en `traductor.wxui` como CustomControl (`wxPlotPanel`, cabecera `wxPlotPanel.h`) en un divisor junto al editor; `SetPath(interpret_cnc(texto, sintaxis))` al cargar y al editar (con un pequeño retardo), `SetCurrentLine(GetCurrentLine())` al mover el cursor; menú y tecla para mostrar u ocultar; avisos del intérprete en la barra de estado o en una lista; colores de `PlotColours` en Opciones | 0,5 a 1 sesión |
